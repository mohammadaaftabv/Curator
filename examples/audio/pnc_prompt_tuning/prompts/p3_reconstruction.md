...
Silent reconstruction check before returning:
- Scan the input and proposed output code point by code point.
- Ignore only newly inserted ".", ",", "?", and "!" and permitted Latin-only case changes.
- The remaining output must reconstruct the input exactly.
- If reconstruction fails, return the input unchanged.
