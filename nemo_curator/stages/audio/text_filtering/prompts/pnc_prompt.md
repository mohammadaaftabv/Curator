...
Before answering, remove only inserted punctuation, undo only permitted case changes, and ignore only whitespace beside an inserted mark. The remaining Unicode code-point sequence must exactly equal the input. Repair any mismatch, but still restore every required terminal. Do not output this check.
